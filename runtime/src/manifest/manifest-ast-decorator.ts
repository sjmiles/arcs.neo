...
export class ManifestAstDecorator {
  ...
  static decorate_recipe(manifest, item: AstNode.RecipeNode) {
    const recipe = {
      name: item.name,
  ...
  static async processItems(items, kind: string, f: Function) {
  ...
  static augmentWithTypes(item: AstNode.All): void {
  ...
  // private static _buildAnnotationRefs(manifest, annotationRefItems: AstNode.AnnotationRef[]): AnnotationRef[] {
