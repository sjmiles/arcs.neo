...
type Particle = any;
type Handle = any;
type AnnotationRef = any;
type IngressValidation = any;
...
export class PlanGenerator {
  ...
  async createPlans(): Promise<string[]> {
    ...
    for (const recipe of this.recipes) {
      ...
      const gps: string[] = await Promise.all(particles.map(p => this.generateParticle(p)));
      //
...
//   }
  ...
  async generateParticle(particle: Particle): Promise<string> {
